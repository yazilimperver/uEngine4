...
namespace basic_engine {
    Animation::Animation(std::string_view animationName, const std::vector<infra::Rectangle<int32_t>>& frames, const SdlTextureAsset* texture)
    ...
    const infra::Rectangle<int32_t>& Animation::Frame(uint32_t n) const {
        return mFrames[n];
    }
}